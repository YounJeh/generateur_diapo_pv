import { createCanvas, loadImage } from "@napi-rs/canvas";
import { describe, expect, it } from "vitest";
import { openPdfPage } from "../../src/pdf/document.js";
import { findMonthlyEnergyChartBounds } from "../../src/pdf/monthlyEnergyChartBounds.js";
import { renderChartImage } from "../../src/pdf/renderChart.js";

const FIXTURE_PDF =
  "test/data/D_26_1223_Intermarche_Rixhiem_3_Omb_avec_stockage_V2.pdf";

describe("renderChartImage", () => {
  it("produces a non-trivial, correctly-sized PNG of the monthly energy chart", async () => {
    const page = await openPdfPage(FIXTURE_PDF, 3);
    const bounds = await findMonthlyEnergyChartBounds(page);

    const png = await renderChartImage(FIXTURE_PDF, 3, bounds);

    // Signature PNG
    expect(png.subarray(0, 8).toString("hex")).toBe("89504e470d0a1a0a");

    const image = await loadImage(png);
    // La zone fait ~500 x 385 points PDF, rendue à l'échelle x3.
    expect(image.width).toBeGreaterThan(1200);
    expect(image.height).toBeGreaterThan(900);

    // L'image ne doit pas être entièrement blanche (contenu réellement dessiné).
    const canvas = createCanvas(image.width, image.height);
    const context = canvas.getContext("2d");
    context.drawImage(image, 0, 0);
    const { data } = context.getImageData(0, 0, image.width, image.height);
    let nonWhitePixels = 0;
    for (let i = 0; i < data.length; i += 4) {
      if (data[i] !== 255 || data[i + 1] !== 255 || data[i + 2] !== 255) {
        nonWhitePixels++;
      }
    }
    expect(nonWhitePixels).toBeGreaterThan(1000);
  });
});
