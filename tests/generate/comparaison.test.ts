...
describe("buildComparaisonPptx", () => {
  it("assemble un seul groupe (sans-stockage + avec-stockage) en 4 slides de contenu + couverture + conclusion", async () => {
    const result = await buildComparaisonPptx([
      { rangees: 3, pdfSansStockage: PDF_SANS_STOCKAGE, pdfAvecStockage: PDF_AVEC_STOCKAGE },
    ]);

    // 4 slides de contenu + 1 couverture + 1 conclusion.
  ...
  it("assemble N=2 groupes dynamiques en 6 slides de contenu + couverture + conclusion, sans avertissement pour le groupe complet", async () => {
    const result = await buildComparaisonPptx([
      { rangees: 3, pdfSansStockage: PDF_SANS_STOCKAGE, pdfAvecStockage: PDF_AVEC_STOCKAGE },
      { rangees: 2, pdfSansStockage: PDF_SANS_STOCKAGE_2 },
    ]);

    expect(result.totalSlides).toBe(8);
    expect(result.groupes).toHaveLength(2);
    expect(result.groupes[1].cases).toHaveLength(1);
    expect(zipSlideCount(result.zip)).toBe(8);
  });
